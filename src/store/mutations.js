export default {
  ...
  removeStateElmByID(state, payload) {
    // mutate state
    this.state.form[payload.field] = state.form[payload.field].filter(el => {
  ...
  restoreStateBackup(state, payload) {
    ...
    // state <= backups
    this.state.form[payload.field].push(
      Object.assign({}, this.state.backups[payload.field])
    );
  },
