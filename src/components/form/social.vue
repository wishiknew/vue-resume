  ...
  <b-row>
    ...
    <b-col cols="12" sm="5" md="12" lg="5">
      ...
      <small class="text-danger">{{
        validation.firstError("socialMedia")
      }}</small>
    </b-col>
...
export default {
  ...
  computed: {
    ...mapGetters(["getSocialNetworksList"])
  },
  validators: {
    ...
    pseudo: function(value) {
      return Validator.value(value)
        .required()
        .regex("^[a-zA-Z0-9-_]+$", "Must be a valid pseudo.");
    }
  },
  methods: {
    addSocialNetwork() {
      ...
      this.$validate().then(success => {
        if (success) {
          this.$store.commit("appendStateField", {
            ...
            value: {
              icon: this.socialMedia,
              pseudo: this.pseudo,
              url:
                this.getSocialNetworksList.find(
                  item => item.value === this.socialMedia
                ).url + this.pseudo
            }
          });
          this.onReset();
